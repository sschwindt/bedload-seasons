...
def copy_excel_cell_range(
    ...
    # this comes from https://stackoverflow.com/a/34838233/5741205
...
def append_df_to_excel(
    ...
    """
    Append a DataFrame [df] to existing Excel file into [sheet_name] Sheet.
    ...
    Function provided by MaxU (https://stackoverflow.com/users/5741205/maxu?tab=profile)
    """
...
def write_xlsx(filename, sheet_name, dataframe, replace_sheet=False, **kwargs):
    """ Soft xlsx handling: helper function to avoid that pd.to_excel overwrites everything in an existing xlsx
        file
    :param str filename: path + name of an xlsx file (e.g., 'sub/folder/a-table.xlsx')
    :param str sheet_name: name of a sheet where data should be written (e.g., 'summary-sheet')
    :param pd.DataFrame dataframe: pandas DataFrame to be written to the xlsx file (direct pass-through)
    :param bool replace_sheet: default is False; set to True to overwrite an existing sheet with the same name.
    :param kwargs: optional keyword arguments that pd.df.to_excel supports
    """
    # define mode depending on if a file exists already
    ...
    with pd.ExcelWriter(filename, engine='openpyxl', mode=mode) as writer:
        ...
        try:
            if 'a' in mode:
                append_df_to_excel(filename, dataframe, sheet_name=sheet_name, index=False, **kwargs)
            else:
                dataframe.to_excel(writer, sheet_name=sheet_name, index=False, **kwargs)
            writer.save()
            print('Updated ' + str(filename))
        except Exception as e:
            print('Could not write DataFrame to XLSX:\n' + str(e))
            return -1
    return 0
